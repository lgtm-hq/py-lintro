...
@pytest.fixture
def make_tools() -> Callable[[dict[str, list[str]]], dict[str, BaseToolPlugin]]:
    ...
    def _build(spec: dict[str, list[str]]) -> dict[str, BaseToolPlugin]:
        # The selection helpers only read ``definition.file_patterns``, so the
        # structural stand-in is cast rather than subclassing the real plugin.
        return {
            name: cast(
                BaseToolPlugin,
                _FakePlugin(definition=_FakeDefinition(file_patterns=patterns)),
            )
