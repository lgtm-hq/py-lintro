...
class TestRiskToSarifLevel:
    """Tests for risk level to SARIF level mapping."""

    def test_high_maps_to_error(self) -> None:
        """Map high risk to error level."""
        assert_that(_risk_to_sarif_level("high")).is_equal_to("error")

    def test_critical_maps_to_error(self) -> None:
        """Map critical risk to error level."""
        assert_that(_risk_to_sarif_level("critical")).is_equal_to("error")

    def test_medium_maps_to_warning(self) -> None:
        """Map medium risk to warning level."""
        assert_that(_risk_to_sarif_level("medium")).is_equal_to("warning")

    def test_behavioral_risk_maps_to_warning(self) -> None:
        """Map behavioral-risk to warning level."""
        assert_that(_risk_to_sarif_level("behavioral-risk")).is_equal_to("warning")

    def test_low_maps_to_note(self) -> None:
        """Map low risk to note level."""
        assert_that(_risk_to_sarif_level("low")).is_equal_to("note")

    def test_safe_style_maps_to_note(self) -> None:
        """Map safe-style to note level."""
        assert_that(_risk_to_sarif_level("safe-style")).is_equal_to("note")

    def test_empty_defaults_to_warning(self) -> None:
        """Default empty risk to warning level."""
        assert_that(_risk_to_sarif_level("")).is_equal_to("warning")

    def test_case_insensitive(self) -> None:
        """Handle case-insensitive risk labels."""
        assert_that(_risk_to_sarif_level("HIGH")).is_equal_to("error")
...
class TestConfidenceToScore:
    """Tests for confidence label to score mapping."""

    def test_high_confidence(self) -> None:
        """Score high confidence as 0.9."""
        assert_that(_confidence_to_score("high")).is_equal_to(0.9)

    def test_medium_confidence(self) -> None:
        """Score medium confidence as 0.6."""
        assert_that(_confidence_to_score("medium")).is_equal_to(0.6)

    def test_low_confidence(self) -> None:
        """Score low confidence as 0.3."""
        assert_that(_confidence_to_score("low")).is_equal_to(0.3)

    def test_unknown_confidence(self) -> None:
        """Default unknown confidence to 0.5."""
        assert_that(_confidence_to_score("unknown")).is_equal_to(0.5)

    def test_empty_confidence(self) -> None:
        """Default empty confidence to 0.5."""
        assert_that(_confidence_to_score("")).is_equal_to(0.5)
...
class TestToSarif:
    """Tests for SARIF document generation."""

    def test_empty_suggestions_produces_valid_sarif(self) -> None:
    ...
    def test_single_suggestion_produces_result(self) -> None:
        ...
        assert_that(rules[0]["id"]).is_equal_to("B101")
        ...
        result = results[0]
        assert_that(result["ruleId"]).is_equal_to("B101")
    ...
    def test_multiple_suggestions_same_rule_deduplicates_rules(self) -> None:
    ...
    def test_different_rules_create_separate_entries(self) -> None:
    ...
    def test_risk_level_maps_correctly(self) -> None:
    ...
    def test_summary_attached_as_run_properties(self) -> None:
    ...
    def test_no_file_omits_locations(self) -> None:
    ...
    def test_no_suggested_code_omits_fixes(self) -> None:
    ...
    def test_tool_name_in_rule_properties(self) -> None:
...
class TestRenderFixesSarif:
    """Tests for SARIF JSON string rendering."""
    ...
    def test_empty_suggestions_returns_valid_json(self) -> None:
...
class TestWriteSarif:
    """Tests for SARIF file writing."""

    def test_writes_valid_sarif_file(self, tmp_path: Path) -> None:
    ...
    def test_writes_with_summary(self, tmp_path: Path) -> None:
