...
def test_metadata_summary_and_fixes_coexist():
    ...
    assert_that(result.metadata["summary"]["overview"]).is_equal_to("Overview")  # type: ignore[index]  # assertpy is_not_none narrows this
...
def test_metadata_fixed_count_is_attached_and_normalized():
    ...
    assert_that(result.metadata["fixed_count"]).is_equal_to(3)  # type: ignore[index]  # assertpy is_not_none narrows this
    assert_that(result.metadata["applied_count"]).is_equal_to(3)  # type: ignore[index]  # assertpy is_not_none narrows this
