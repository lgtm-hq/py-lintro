...
def test_docker_ci_defers_ci_tag_cleanup_until_consumers_finish() -> None:
    """Cleanup runs only when every ci-tag consumer finished green (#1138).

    Immediate deletion on run completion broke "Re-run failed jobs": docker-build
    does not rebuild on a partial rerun, so downstream jobs pulled a missing tag.
    Main keeps cleanup-ci-images but gates it on no failure/cancelled among needs
    (including publish), so failed runs leave tags for reruns.
    """
    docker_ci = _load_workflow(name="docker-ci.yml")
    jobs = docker_ci["jobs"]
    cleanup = jobs["cleanup-ci-images"]
    assert_that(cleanup["needs"]).contains(
        "docker-build",
        "dogfooding-lint",
        "dogfooding-lint-changed",
        "security-audit",
    ...
    assert_that(condition).contains("!contains(needs.*.result, 'failure')")
    assert_that(condition).contains("!contains(needs.*.result, 'cancelled')")
    run_steps = [
        step.get("run", "")
...
def test_ghcr_cleanup_sweeps_ephemeral_ci_tags() -> None:
    """Scheduled maintenance owns age-based CI-tag sweep for orphaned tags."""
