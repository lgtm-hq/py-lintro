...
# Exact pin: uvx has no lockfile here, so an unpinned `--from sigstore` would
# resolve whatever PyPI publishes on the day of the tag push and could break
# signing mid-release. Kept current by the `SIGSTORE_VERSION` customManager in
# renovate.json.
SIGSTORE_VERSION=4.5.0
...
for artifact in "${artifacts[@]}"; do
	...
	# the pattern the Scorecard Signed-Releases check looks for (it does not
	# match sigstore's default `.sigstore.json`, hence `--no-default-files` to
	# avoid shipping both). `--overwrite` keeps re-runs idempotent.
