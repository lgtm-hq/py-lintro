...
| `verify_built_binary.sh`      | Verify a built binary: `--version`, every exported command run once, the pty AI review, an MCP `initialize` + `tools/list` round trip over stdio (#2577), `--help` | `./scripts/build/verify_built_binary.sh dist/nuitka/lintro`              |
| `drive_mcp_round_trip.py`     | Start the built binary's `lintro mcp` over stdio and complete one JSON-RPC `initialize` + `tools/list` round trip (#2577)                                          | `python3 scripts/build/drive_mcp_round_trip.py dist/nuitka/lintro`       |
...
| `verify_release_wheel.sh`   | Verify the PyPI wheel against the release manifest + attestation | `GH_TOKEN=… VERSION=1.2.3 ATTESTATION_REPO=… DIST_SIGNER_WORKFLOW=… ./scripts/ci/mirror/verify_release_wheel.sh` |
...
| `maintenance/report-workflow-commit-status.sh` | Report a scheduled workflow's verdict as a commit status on main (#2603)                                                         | `STATUS_CONTEXT=ghcr-cleanup JOB_RESULTS=success ./scripts/ci/maintenance/report-workflow-commit-status.sh`                |
...
| `check-tools-manifest-staleness.sh`            | Refuse promoting a tools-image candidate built from a stale tool manifest                                                        | `CANDIDATE_SHA=<sha> MAIN_SHA=<sha> ./scripts/ci/check-tools-manifest-staleness.sh`                                        |
...
| `resolve-image-digest.sh`                      | Resolve the nightly dogfood image and the manifest ref to verify it against (#2602)                                              | `COMMIT_SHA=<sha> ./scripts/ci/resolve-image-digest.sh --help`                                                             |
...
| `verify-image-attestations.sh`                 | Verify the GitHub build-provenance attestation of each staged image digest before promotion (#2562)                              | `IMAGES=$'<image@digest>\n<image@digest>' ./scripts/ci/verify-image-attestations.sh` (one ref per line)                    |
...
| `request-coderabbit-review.sh`                 | Post `@coderabbitai review` on a bot-authored PR with a human PAT (#2726)                                                        | `GH_TOKEN=... REPO=o/r PR_NUMBER=1 ./scripts/ci/request-coderabbit-review.sh`                                              |
...
| `resolve_review_request.py`                    | Gate an `@lintro review` PR comment: write access, open same-repo PR (#2795)                                                     | `COMMENT_BODY=... COMMENTER=u COMMENT_ID=1 PR_NUMBER=1 REPO=o/r GH_TOKEN=... python3 scripts/ci/resolve_review_request.py` |
...
| `run_smoke.py`                                 | Weekly live API smoke for one provider table row (#2600)                                                                         | `python3 scripts/ci/ai_provider_smoke/run_smoke.py --emit-matrix`                                                          |
| `http_capture.py`                              | Record the SDK's HTTP exchange and describe an empty smoke answer (#2748)                                                        | imported by `run_smoke.py`                                                                                                 |
| `post_error_details.py`                        | Quote provider smoke errors on the deduplicated tracker issue (#2600)                                                            | `python3 scripts/ci/ai_provider_smoke/post_error_details.py --help`                                                        |
| `report-commit-status.sh`                      | Report one provider smoke row as a commit status on main (#2600)                                                                 | `SMOKE_NAME=anthropic-api ./scripts/ci/ai_provider_smoke/report-commit-status.sh`                                          |
| `record_cli_fixture.sh`                        | Re-record an agent-CLI replay fixture from the real binary (#2600)                                                               | `./scripts/ci/record_cli_fixture.sh claude`                                                                                |
...
| `check-release-version-skew.py`                | Alarm on post-release PyPI/npm/Homebrew version skew; Homebrew version read from the formula's release url (#1712)               | `python3 scripts/ci/check-release-version-skew.py --help`                                                                  |
| `check-last-publish-green.py`                  | Gate the release version PR on the last version-tag publish being green (#2516)                                                  | `python3 scripts/ci/check-last-publish-green.py --help`                                                                    |
...
| `release-fault.sh`                             | Fail a release step only when `RELEASE_FAULT` names it (#2633 fault injection)                                                   | `RELEASE_FAULT=fail-build scripts/ci/release-fault.sh fail-build`                                                          |
