...
class CheckovIssue(BaseIssue):
    ...
    resource. The native severity and guideline URL come from platform metadata
    that ``--skip-download`` suppresses, and lintro passes that flag on every
    run, so in practice both are ``None`` and lintro falls back to the
    inherited default severity and a static documentation URL. Both fields are
    still read so nothing is lost if a report ever carries them.
    ...
    def __post_init__(self) -> None:
        ...
        # Checkov only emits a guideline URL from downloaded platform metadata.
    ...
    def _get_message(self) -> str:
        ...
        """
        # A report missing check_name would otherwise render as a bare
        # " (resource: …)" clause with nothing to say what failed; the check id
        # is always present and is the next most specific thing to name.
        description = self.check_name or self.check_id
        if self.resource:
            return f"{description} (resource: {self.resource})"
        return description
