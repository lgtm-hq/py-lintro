...
class ObserverLike(Protocol):
    """Minimal watchdog-``Observer`` surface used by :func:`watch_paths`.

    Declared as a protocol so tests can inject a lightweight fake observer
    without subclassing watchdog's threading machinery.
    """
    ...
    def schedule(
        ...
        *,
        ...
        """Register ``event_handler`` for events under ``path``.

        Args:
            event_handler: Handler receiving filesystem events.
            path: Directory (or file) to watch.
            recursive: Whether to watch subdirectories.

        Returns:
            An opaque watch handle.
        """
    ...
    def join(self, timeout: float | None = None) -> None:
        """Wait for the observer to finish shutting down.
