...
- **Calibrate severity.** P1 means merge-blocking defect: expect 0–2 on a typical PR and
  ...
  exists yet. A test gap is P3 unless the PR claims to fix a bug it does not test — and
  then the missing test is visible in the diff, so say `evidence_style: diff_local`.
  ...
  finding at P2 must carry `evidence_style: diff_local`; one that does not, or that
  leaves the field out, is automatically moved to P3 and the correction is recorded
  against the run.
