...
def format_pass_note_lines(*, metadata: ReviewMetadata) -> list[str]:
    ...
    Returns:
        ``["", note, ...]`` for each note that applies; empty when none do,
        so a default round's block is byte-identical to before.
    """
    notes = (
