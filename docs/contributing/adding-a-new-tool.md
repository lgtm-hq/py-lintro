...
2. **`pyproject.toml`** — add to the appropriate optional-dependency group (typically
   ...
   Exception: semgrep is **not** in any extra. Pin it in `requirements-semgrep.in`
   and recompile the hash-pinned lockfile:
   ...
   `scripts/ci/generate-tool-versions.py` so the generator still reads the pin
   from `requirements-semgrep.txt`.
