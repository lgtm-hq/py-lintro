...
- **No `--bc-api-key` is ever passed** — without an API key Checkov performs no results
...
`severity = None` and normalize to lintro's default `WARNING`. When a key is present,
native `CRITICAL`/`HIGH`/`MEDIUM`/`LOW` values are honored via lintro's severity alias
...
`scripts/utils/install-tools.sh` performs the isolated install via `uv tool install`,
placing the `checkov` shim on `PATH` while keeping its dependencies out of lintro's own
