...
djLint has a floor of `1.44.2` and no ceiling (see the `tools` extra in
`pyproject.toml`). Earlier drafts of this plugin capped djLint below `1.39.3` because
djLint requires `click>=8.2` from that release on, which collided with semgrep's
`click~=8.1.8` pin. Semgrep now installs from the isolated `requirements-semgrep.txt`
lockfile, so its pins no longer constrain lintro's shared resolver and the ceiling is
gone.
