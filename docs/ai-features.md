...
- `GitHub secondary rate limit (HTTP 403)` — the token was throttled for content
  creation; the next round posts the comments.
...
- `this token is not permitted to post reviews on this PR (HTTP 403)` — a 401/403 that
  is not a rate limit.
