...
only the flag defaults — `ai.provider` and `ai.transport` are still required whenever
`ai.lint` or `ai.review` is enabled, so merge it with the complete example in
